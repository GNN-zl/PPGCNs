...
def calc_heat_kernel_polynomials(adj, Kg, t):
    ...
    I_np = np.identity(n)
    if Kg - 1 == 0:
        heat_kernel_polynomials = [I_np]
    else:
        heat_kernel_polynomials = [I_np]
        for k in range(1, Kg):
            k_th = expm(-t * k * lap)
            heat_kernel_polynomials.append(k_th)
    return heat_kernel_polynomials

def B_adj_inverse(adj):
    adj = sp.coo_matrix(adj)
    adj = adj + sp.eye(adj.shape[0])
    row_sum = np.array(adj.sum(1))
    row_sum = (row_sum==0)*1 + row_sum
    B = (1/2) * row_sum * (row_sum -1)
    inv_B = np.power(row_sum, -1).flatten()
    inv_B[np.isinf(inv_B)] = 0.
    inv_mat_B = np.diag(inv_B)

    return torch.FloatTensor(inv_mat_B)
...
def load_citation(dataset_str="cora"):
    ...
    return adj, features, labels, idx_train, idx_val, idx_test,  adj_origin, graph



def load_highfrequency(dataset_str="cornell"):
    graph_adjacenct_list_file_path = 'new_data/{}/out1_graph_edges.txt'.format(dataset_str)
    graph_node_features_and_labels_file_path = 'new_data/{}/out1_node_feature_label.txt'.format(dataset_str)
    ...
    return adj, features, labels, train, val, test, adj_origin, graph
...
if __name__ == '__main__':
    #compute_feature_smoothness()
    #compute_label_smoothness()
    #load_npzdataset()
    #load_citation()
    test()
'''